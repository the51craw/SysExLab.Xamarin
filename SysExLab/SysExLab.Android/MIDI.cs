...
using Java.Lang;
using JniNativeMethod = Android.Runtime.JNIEnv;
...
namespace SysExLab.Droid
{
    public class MIDI
    {
        ...
        MidiOutputPort midiOutPort;
        MidiInputPort midiInPort;
        ...
        public MIDI(MainPage mainPage, Picker OutputDeviceSelector, Picker InputDeviceSelector, /*CoreDispatcher Dispatcher, */byte MidiOutPortChannel, byte MidiInPortChannel)
        {
            ...
            IntPtr class_ref = JNIEnv.FindClass("mono/android/MIDI");

            //Mono.Runtime.

            //Java.Interop.TypeManager.

            //Android.Runtime.JNIEnv jNIEnv;
            //Android.Runtime.JNIEnv.ToJniHandle()

            //MidiDeviceService midiDeviceService;
            //MidiDeviceService.GetObject<MidiManager>()


            //PackageManager packageManager = PackageManager.GetObject(JNIEnv.ToJniHandle, App.Current.MainPage, Android.Runtime.JniHandleOwnership.TransferGlobalRef);
            //if (!PackageManager.HasSystemFeature(PackageManager.FeatureMidi)) throw new NotSupportedException("Midi");
            //var midiManager = (MidiManager)GetSystemService(MidiService);
            //var midiDevInfos = (MidiDeviceInfo[])midiManager.GetDevices();

            //MidiDevice.GetObject(Java.Interop.JniEnvironment.)
            MidiDevice midiDevice = DependencyService.Get<MidiDevice>(DependencyFetchTarget.GlobalInstance);
            MidiDevice midiDevice2 = DependencyService.Get<MidiDevice>(DependencyFetchTarget.NewInstance);
            MidiDevice.MidiConnection midiConnection = DependencyService.Get<MidiDevice.MidiConnection>(DependencyFetchTarget.NewInstance);
            //midiConnection.Close();
            //midiDevice.Close();
            ////MidiDevice.GetObject(jnienv: , handle:)
            //MidiDeviceInfo.GetObject(Java.Interop.JniEnvironment. )
            ////MidiDeviceInfo.InterfaceConsts.ContentsFileDescriptor contentsFileDescriptor;
            //MidiDeviceInfo.PortInfo portInfo;
            //MidiDeviceService midiDeviceService;
            ////MidiDeviceService.InterfaceConsts. interfaceConsts;
        ...
        public async void Init(string deviceName)
        {
            ...
            //foreach (Android.Media.Midi.MidiOutputPort outPort in Android.Media.Midi.MidiManager)
